...
export default function BlogHomeScreen() {
  ...
  const visiblePosts = useMemo(() => {
    ...
    for (const post of rawVisiblePosts) {
      ...
      const existingTime = existing?.date
  ...
  const updateCategoryTabsPinnedForScroll = useCallback(
    (currentY: number) => {
      ...
      const pinAt = Math.max(
        0,
        categoryTabsOffsetY + CATEGORY_TABS_PIN_HYSTERESIS,
      );
      const unpinAt = Math.max(
        0,
        categoryTabsOffsetY - CATEGORY_TABS_PIN_HYSTERESIS,
      );
  ...
  const handleScroll = useCallback(
    (event: any) => {
      ...
      if (scrollDeltaAccumulatorRef.current > AUDIO_BAR_SCROLL_HIDE_THRESHOLD) {
    ...
    >
      <SafeArea>
        ...
        <View className="flex-1 relative">
          {categoryTabsPinned ? (
            <View
              className="absolute left-0 right-0 top-0 border-b bg-background"
              style={{
                backgroundColor: colors.background,
                borderBottomColor:
          ...
          <LegendList
            ...
            ListHeaderComponent={
              <View>
                ...
                <Text
                  className="px-4 pb-3 pt-5 text-xl font-extrabold text-foreground"
