    ...
    <View className="flex-1 bg-background">
      <SafeArea>
        <View className="flex-row items-center gap-3 px-4 py-3">
          ...
          <Text className="flex-1 text-lg font-bold text-foreground">Playlists</Text>
          <Pressable
            ...
            renderItem={({ item }) => (
              ...
              >
                <View className="size-12 items-center justify-center rounded-lg bg-muted">
                  <Icon name="ListMusic" size={22} className="text-muted-foreground" />
                </View>
      ...
      <Modal visible={createVisible} transparent animationType="fade" onRequestClose={() => setCreateVisible(false)}>
        <Pressable className="flex-1 justify-center bg-black/60 px-5" onPress={() => setCreateVisible(false)}>
          <Pressable onPress={(event) => event.stopPropagation()} className="rounded-2xl bg-card p-4">
            <Text className="mb-3 text-base font-bold text-foreground">Create playlist</Text>
            <View className="mb-4 flex-row items-center gap-2 rounded-xl border border-border bg-muted px-3">
              <Icon name="ListMusic" size={16} className="text-muted-foreground" />
              <TextInput
                value={name}
                ...
                style={{ flex: 1, color: colors.foreground, fontSize: 14, paddingVertical: 12 }}
              />
            </View>
            <View className="flex-row gap-2">
              <Pressable onPress={() => setCreateVisible(false)} className="h-11 flex-1 items-center justify-center rounded-xl bg-muted">
