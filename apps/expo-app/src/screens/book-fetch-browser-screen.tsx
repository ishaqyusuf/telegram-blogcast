...
import { ChapterImportProgress } from "@/components/book/chapter-import-progress";
...
export default function BookFetchBrowserScreen() {
	const { url, bookId, returnPageId } = useLocalSearchParams<{
		url?: string;
		bookId?: string;
		returnPageId?: string;
	}>();
	...
	const [savedPage, setSavedPage] = useState<{
		...
		bookId &&
		url &&
		/^https:\/\/shamela\.ws\/book\/\d+\/?$/.test(url)
			? { bookId: Number(bookId), pageId: Number(returnPageId), rootUrl: url }
			: null,
	);
	const [chapterImportId, setChapterImportId] = useState<string | null>(null);
	...
	const { mutate: captureChapters, isPending: isSavingChapters } = useMutation(
		_trpc.bookChapter.capture.mutationOptions({
			onSuccess: (job) => {
	...
	const canCapture = useMemo(() => {
		if (chapterImportId) return false;
					...
					{chapterImportId ? (
						<View className="flex-1 px-4 pt-4">
							<ChapterImportProgress
								importId={chapterImportId}
								onComplete={openSavedPage}
								onViewPage={openSavedPage}
							/>
						</View>
					) : (
