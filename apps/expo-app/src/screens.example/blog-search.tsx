										...
										<View className="flex-row items-center justify-between gap-3">
											<View className="min-w-0 flex-1">
												...
												<Text
													...
													numberOfLines={1}
												>
													{channel.username ? `@${channel.username}` : "Channel"}
												</Text>
											</View>
											{isSelected ? (
												<Icon
...
export default function BlogSearch({ onBackPress }: BlogSearchProps) {
	...
	const selectedChannels = useMemo(
		() => channels.filter((channel: any) => selectedChannelIds.includes(channel.id)),
				...
				{query.length > 0 || hasFilters ? (
					...
					>
						<View className="mb-6">
							...
							{["Digital Nomadism", "Mindfulness Meditation", "Kyoto Travel Guide"].map(
								(term) => (
									<View
										key={term}
