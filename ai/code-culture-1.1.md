...
### Data List View (Using `Table.Provider`, No `<Table>`) — `components/tables/example/data-table.tsx`
```ts
"use client";
...
export function DataTable(props: Props) {
    ...
    return (
        <Table.Provider
            args={[{
                columns: [], // optional when rendering custom list rows
                data,
                checkbox: true,
                tableScroll,
                rowSelection,
        ...
        >
            <div className="flex flex-col gap-4 w-full">
                ...
                {/* Custom list view: render data.map directly inside Provider */}
...
List View Rules:
- Must still wrap content in `Table.Provider` to reuse shared table state/actions.
- Do not render `<Table>` when screen requires card/list UX.
- Render items using `data.map(...)` inside Provider.
- Keep `Table.LoadMore` and batch actions if infinite/batch behaviors are needed.
...
## 🔟 Planning & Progress Documentation (Mandatory)

### 10.1 Canonical Plan File
- Always use `ai/plan.md` as the single source of truth for planning and delivery tracking.
- For every new task, update `ai/plan.md` before implementation starts.

### 10.2 Required Updates During Work
- Update **Execution Plan** when scope/tasks change.
- Update **Progress Log** after each meaningful implementation step.
- Update **Blockers & Risks** immediately when an issue is discovered.
- Update **Decisions** whenever a technical/product tradeoff is made.
- Update **Roadmap** when priorities or sequencing changes.

### 10.3 Minimum Standard Per Request
- Every request must include:
  1. Plan entry (what will be done)
  2. Progress update (what was done)
  3. Next step / roadmap placement (what comes next)

### 10.4 Completion Rule
- Before marking work complete, ensure `ai/plan.md` reflects:
  - final status,
  - implemented scope,
