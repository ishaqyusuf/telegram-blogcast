...
- The Facebook bridge size contract is inclusive at 20 MiB for normal in-app media, Telegram-message external playback above 20 MiB through 50 MiB, and Facebook external playback above 50 MiB without a full media download/upload. `posts` and `blog.search` expose normalized `externalMedia` data so Expo never attempts Telegram `getFile` for these items.
...
- `album.getSuggestedMedia` accepts `{ albumId, channelId?, limit?, keyword? }` and returns channel-hydrated audio candidates with matching-term metadata while excluding media already in the album. An album's persisted or inferred channel is authoritative; `channelId` supplies suggestion scope only for an empty, channel-less album.
...
- `bookChapter.capture({ bookId, pageId, finalUrl, html })` accepts a matching HTTPS Shamela root and an already-saved page. It persists an idempotent capture (maximum 3 MB UTF-8), then enqueues Trigger. Acceptance returns import status, not a completed tree. The worker validates completeness and publishes atomically without page stubs or content changes.
- `book.captureShamelaChapters` is a compatibility adapter to the durable route, returning importId/status with zero immediate node/stub counts. New clients use bookChapter directly.
- Capture/retry/cancel require a 64-hex per-install bearer capability in `x-book-import-token`; only its SHA-256 hash is stored. Only the originating installation can retry/cancel. This is job ownership, not account authentication. Missing-token legacy clients must update. Capture is limited to ten unique captures per book per hour; duplicate captures return the saved job; retry is limited to five generations.
- A terminal capture owned by another installation, or with exhausted generations, permits a bounded replacement capture. Its old deduplication key is archived as hash:importId; the original row and ownership remain intact. The replacement gets the current content hash and deduplicates subsequent requests.
- `bookChapter.status`, `latest`, and `bookState` expose progress without raw HTML or ownership hashes. Status includes `canManage`. `retry` reuses retained HTML under a new generation; `cancel` changes job state independently of the long book transaction, whose final compare-and-set rolls back cancelled publication.
- `bookChapter.list` uses a cursor, parentId, optional global title/page-number query, and a default 40/max 100 rows. `resolvePage` checks saved content freshly before returning either local pageId or derived sourceUrl.
- `book.getBook({ id, includeToc: false })` omits the full chapter tree for mobile detail; the separate virtualized chapter screen uses the paginated API. Existing callers default to includeToc true.
