...
- **Preview/production local network launch gate** - added a keyboard-safe cold-launch IP selector with filtered recent history, transient enable/disable state, selected-IP Telegram client, guarded Facebook/import/transcription routes and observers, and Settings re-enable behavior. Plan: `brain/plans/2026-07-20-feature-preview-production-local-network-launch.md`.
- **Audio transcript streaming and local services IP settings** - added saved-transcript window loading, virtualized read mode rendering, playback-nearby transcript prefetch, saved-transcript-first chunk behavior, shared Local Services IP settings/history, IP-derived local API/transcriber/Facebook URLs, and silent startup degradation for unreachable local channel-update services. Plan: `brain/plans/2026-07-10-feature-audio-transcript-streaming-karaoke-local-service-settings.md`.
- **Physical Library catalog** - added a Books-area physical home-library catalog with dedicated Prisma models, `library` tRPC CRUD/link/search procedures, Expo list/detail/create/edit routes, label/location metadata, purchase/shelf/volume fields, and digital-book linking from both physical entries and digital book detail. Plan: `brain/plans/2026-07-04-feature-physical-library-catalog.md`.
